...
// --- 3. COMPONENTE: SECCIÓN DE COMENTARIOS (USANDO COLUMNA 'NOMBRE') ---
const CommentsSection = ({ imagenId }) => {
  ...
  const enviarComentario = async (e) => {
    ...
    setEnviando(true);
    try {
      // CAMBIO: Ahora enviamos 'nombre' en lugar de 'perfil_id'
      const { data, error } = await supabase
        .from('comentarios')
        .insert([{ 
          texto: nuevoComentario.trim(), 
          user_id: user.id, 
          nombre: perfil?.nombre || "Usuario", // Guardamos el nombre actual del usuario
      ...
      if (error) {
        ...
        alert("Asegúrate de que la columna 'nombre' existe en la tabla comentarios");
      } else if (data) {
      ...
      {user ? (
        <form onSubmit={enviarComentario} className="mb-12 flex gap-3">
          ...
          <button 
            type="submit"
            disabled={enviando}
            className="bg-white text-black text-[10px] font-black px-8 py-2 rounded-xl hover:bg-gray-200 disabled:opacity-50 transition-all"
          >
            {enviando ? "..." : "PUBLICAR"}
          </button>
        </form>
      ) : (
        <div className="mb-12 p-8 bg-white/[0.02] rounded-2xl border border-dashed border-white/10 text-center text-white/20 text-[10px] uppercase tracking-[0.2em]">
          ...
          <div key={c.id} className="group animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center mb-3">
              <span className="text-white text-[11px] font-black uppercase tracking-tight">
                {/* Mostramos el nombre guardado en la columna 'nombre' */}
                {c.nombre || "Usuario"}
              </span>
...
// --- 4. VISUALIZADOR PRINCIPAL (MINIATURAS GRANDES Y CERCANAS) ---
    ...
    <div className="fixed inset-0 z-[9999] flex flex-col bg-black selection:bg-white selection:text-black overflow-y-auto no-scrollbar">
      ...
      <div className="flex flex-col lg:flex-row w-full max-w-[2000px] mx-auto flex-1">
        <div className="flex-1 flex flex-col items-center w-full lg:border-r lg:border-white/5">
          {/* ARTE */}
          <div className="relative flex items-center justify-center w-full min-h-[50vh] md:min-h-[85vh] px-4 py-6">
            <button className="absolute left-2 z-[105] text-white/10 hover:text-white text-6xl md:text-9xl p-2 transition-all active:scale-90" onClick={() => setCurrentIndex((currentIndex - 1 + gallery.length) % gallery.length)}>‹</button>
            <img key={selectedImg.src} src={selectedImg.src} alt={selectedImg.alt} className="max-h-[70vh] md:max-h-[80vh] max-w-full object-contain shadow-2xl rounded-sm animate-in zoom-in-95 duration-700" />
            <button className="absolute right-2 z-[105] text-white/10 hover:text-white text-6xl md:text-9xl p-2 transition-all active:scale-90" onClick={() => setCurrentIndex((currentIndex + 1) % gallery.length)}>›</button>
          </div>

          {/* MINIATURAS MÓVIL (GRANDES H-24) */}
          <div className="lg:hidden w-full px-6 mb-12">
            <div className="flex gap-4 overflow-x-auto no-scrollbar py-6 border-y border-white/5 bg-white/[0.01] rounded-2xl px-5">
              {gallery.map((img, idx) => (
                <button key={idx} onClick={() => setCurrentIndex(idx)} className={`relative h-24 w-24 min-w-[96px] rounded-xl overflow-hidden transition-all duration-500 ${idx === currentIndex ? 'ring-2 ring-white scale-105 opacity-100 shadow-2xl' : 'opacity-20 grayscale'}`}>
        ...
        <div className="hidden lg:flex flex-col items-center gap-5 p-6 bg-white/[0.01] sticky top-[96px] h-[calc(100vh-96px)] overflow-y-auto no-scrollbar w-[120px]">
          {gallery.map((img, idx) => (
            <button key={idx} onClick={() => setCurrentIndex(idx)} className={`relative h-24 w-24 min-h-[96px] rounded-2xl overflow-hidden transition-all duration-500 ${idx === currentIndex ? 'ring-2 ring-white scale-110 opacity-100 shadow-2xl z-10' : 'opacity-10 grayscale hover:opacity-100'}`}>
              <img src={img.src} className="h-full w-full object-cover" alt="thumb" />
            </button>
          ))}
        </div>
